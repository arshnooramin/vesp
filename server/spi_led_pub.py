...
client.publish(peripheralTopic + "bus_add_device",
               ",".join(["1", "0", "0", "0", "3", "1", "1000000", "7", "2", "0"]), qos=qos)
time.sleep(1)
...
while True:
    ...
    for i in range(1, 16):
        ...
        rgb_arr[i*4 + 2] = str(0)
        rgb_arr[i*4 + 3] = str(0)
    rgb_arr[counter*4 + 1] = str(255)
    rgb_arr[counter*4 + 2] = str(0)
    rgb_arr[counter*4 + 3] = str(0)
    rgb_arr_str = length + ",0,2," + ",".join(rgb_arr)
    client.publish(peripheralTopic + "device_transmit", rgb_arr_str, qos=qos)
    counter += 1
